...
public class Test {
	...
	public static void main(String[] args) {
		...
		String damorin = "newDamorin.Agent";
		...
		ArcadeMachine.runOneGame(game, level1, visuals, damorin, recordActionsFile, seed);
		...
		// ArcadeMachine.runGames(game, new String[]{level1}, 5,
		// evolutionStrategies, null);
		// }

		// 5. This starts a game, in a generated level created by a specific
		// level generator
		// if(ArcadeMachine.generateOneLevel(game, geneticGenerator,
		// recordLevelFile)){
		...
		/*
		 * int N = 10, L = 1, M = 5; boolean saveActions = false; String[]
		 * levels = new String[L]; String[] actionFiles = new String[L*M];
		 * for(int i = 0; i < N; ++i) { int actionIdx = 0; game = gamesPath +
		 * games[i] + ".txt"; for(int j = 0; j < L; ++j){ levels[j] = gamesPath
		 * + games[i] + "_lvl" + j +".txt"; if(saveActions) for(int k = 0; k <
		 * M; ++k) actionFiles[actionIdx++] = "actions_game_" + i + "_level_" +
		 * j + "_" + k + ".txt"; } ArcadeMachine.runGames(game, levels, M,
		 * kNearestNeighbour, saveActions? actionFiles:null); }
		 */
	}
}
