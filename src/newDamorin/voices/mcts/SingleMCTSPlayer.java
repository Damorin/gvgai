...
public class SingleMCTSPlayer {
	...
	/**
	 * At the end of each time step, we keep the node that corresponds to the
	 * chosen action.
	 */
	...
	public void init(StateObservation gameState) {
		...
		if (salvagedTree == null) { // if there is nothing saved from a previous
									// time step, initialize an empty tree node
			rootNode = new SingleTreeNode();
		} else { // else, initialize the tree with the node that was chosen, and
					// update the memory index for past position arrays
	...
	public int run(ElapsedCpuTimer elapsedTimer) {
		mctsSearch(elapsedTimer, this.rootObservation); // Do the search within
														// the available time.
		int action = rootNode.mostVisitedAction(); // Determine the best action
													// to take and return it.
	...
	private void mctsSearch(ElapsedCpuTimer elapsedTimer, StateObservation rootObservation) {
		...
		while (remaining > 2 * avgTimeTaken && remaining > remainingLimit) {
			...
			// treepolicy: navigate from the root node until either we add a new
			// node or we reach a final state
			...
			selected.backUp(selected, delta); // TODO : I should probably make
												// the backup method cleaner
	 ...
	 * @return the tree node where the tree navigation has ended (it can be a
	 *         final node/state, or just the node where the policy exited the
	 *         tree.
	 */
	private SingleTreeNode treePolicy(StateObservation currentObservation) {
		...
		while (!(currentObservation.isGameOver())) {
			if (currentNode.notFullyExpanded()) {
				...
				currentObservation.advance(Agent.actions.get(next.getActionIndex()));
	...
	/**
	 * Expands the tree, from a given node, and given a certain state
	 * observation
	 * 
	 * @param fatherNode
	 *            the node from which we are expanding the tree
	 * @param currentObservation
	 *            the state observation that we are currently in (for *this*
	...
	private SingleTreeNode expand(SingleTreeNode fatherNode, StateObservation currentObservation) {
		...
		currentObservation.advance(Agent.actions.get(bestAction));
	...
	public double rollOut(StateObservation _currentObservation) {
		...
		while (!finishRollout(_currentObservation, rolloutDepth)) {
			...
			_currentObservation.advance(Agent.actions.get(action));
			rolloutDepth++;
		}
